...
//Disable the stop button since it's not needed until game start.
window.onload = function() {watch()};
function watch() {
    var btn = document.getElementById('btnStop');
    btnDisabled(btn); //disable the stop button since game hasn't started.
}
...
function rollForTurn() {
    ...
    //determine and concatenate string showing which player won roll
    if (pOne > pTwo) {
        first = "Player 1";
...
//initialize the game, roll for turn & determine the active player
function startGame() {
    var xTurn= 0;
    ...
    btnDisabled(btn); // disables start button since the game is now afoot
    var btn = document.getElementById('btnStop');
    stopEnabled(btn); //enables stop button since game is afoot
...
//this function styles the game buttons while they're disabled
...
function stopGame() {
    ...
    var btn = document.getElementById('btnStop');
    btnDisabled(btn); //disable stop button since game is now stopped
    var showPlayer = document.getElementById('showPlayer'); 
    showPlayer.innerHTML = "Game Stopped";
    showPlayer.style.color ='red';

    //reset all squares to their starting empty space
...
//this function will show the msg console and any text it may have
...
function determineAvatar() {
    ...
    return paintAvatar; //returns correct avatar based on player
}
...
function avatarPlaced() {
    ...
    var showPlayer = document.getElementById('showPlayer'); 
    //checks for winner. if there is, don't continue game.
...
//gets array of the current board and checks proposed moved for validity.
...
function recordMoves(square) {
    ...
    var boardState = document.getElementById('boardState').innerHTML; //retrieves boardState Array
    var info = boardState.split(','); //seperate string by comma to create an array
...
function checkForWinCon() {
    ...
    info = info.split(','); //seperate strubg by commas into the array
    info.sort(); // sort the square array in order despie GP sequece;
    for (var i in info) {
...
function check4Tie() {
    ...
    if(boardState.length >=9 && check != "That's three in a row, Player 1 wins!" && check !="That's three in a row, Player 2 wins!") {
        var txt1="Aww, man! Bad news! Nobody wins... But the good news as that nobody loses!<br><br> Still a tie, though. Sorry.";
        tieSound();//sound for tie
        writeMsg(txt1);
...
function winner(winDetected, winCon) {
    if (winDetected=="win") {
        ...
        writeMsg(txt2);
        var btn = document.getElementById('btnStart'); 
        startEnabled(btn);//enable the start button since game is now stopped
        var btn = document.getElementById('btnStop');
        btnDisabled(btn); //disable the stop button since game is now stopped
        document.getElementById('showPlayer').innerHTML = "Game Stopped";
        glowBoard(winCon); //makes gameboard pulse with colors
    }
}
...
//produces games sounds depending on situation
...
function checkWinCon4(info,squareArray) {
    ...
    if (match0Avatar != undefined && match3Avatar == match6Avatar != undefined) {
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") {// if game has not yet start started prevent avatar placement
        ...
        if (verdict == undefined) { //if verdict is empty than the square is unoccupied
            var paintAvatar = determineAvatar(); //get the correct avatar to paint for active player
            var selected = document.getElementsByClassName(paintAvatar)[0]; // paint avatar
            if (paintAvatar == "O") { // change theses all to tenary statements instead
                animateO(selected); // call function to animate O
            } else if (paintAvatar == "X") {
                animateX(selected);
            }
            //build new array adding newly selelcted square and the assgned avatar
